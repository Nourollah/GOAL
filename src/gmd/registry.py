...
The registry is the backbone of GMD's extensibility. It supports three
registration mechanisms simultaneously and uses lazy loading to avoid
import side effects:

1. **Decorator registration** — for built-in classes within gmd itself.
...
class Registry:
    ...
    def register(self, name: str) -> typing.Callable[[typing.Type[T]], typing.Type[T]]:
        ...
        Use this for built-in classes within the gmd package itself.
        Third parties use this in their own modules — triggered by their
        import, not gmd's import. No hidden side effects in gmd's own
        ``__init__`` files.
    ...
    def register_lazy(self, name: str, import_path: str) -> None:
        ...
        Format: ``'gmd.nn.models.hyperspec:HyperSpecModel'``

        Use this for all built-in registrations in gmd — zero import cost.

        Usage::

            MODEL_REGISTRY.register_lazy(
                'hyperspec', 'gmd.nn.models.hyperspec:HyperSpecModel'
            )
        """
    ...
    def discover_plugins(self, group: str) -> None:
        ...
        Call once at startup::
            ...
            MODEL_REGISTRY.discover_plugins('gmd.models')

        Third parties declare in their ``pyproject.toml``::

            [project.entry-points.'gmd.models']
