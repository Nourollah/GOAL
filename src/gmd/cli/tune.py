...
def _build_components(cfg: DictConfig) -> typing.Tuple[typing.Any, typing.Any, CompositeLoss]:
    ...
    head_kwargs: typing.Dict[str, typing.Any] = {
        k: v for k, v in cfg.model.head.items() if k != "name"
    }
...
def _ray_train_fn(config: typing.Dict[str, typing.Any], base_cfg: DictConfig) -> None:
    ...
    module: GMDModule = GMDModule(
        backbone=backbone, head=head, loss=loss, config=merged,
    )
    datamodule: GMDDataModule = GMDDataModule(merged)

    trainer: L.Trainer = L.Trainer(
        max_epochs=merged.hparams_search.get(
            "max_epochs", merged.training.get("max_epochs", 500)
        ),
        accelerator="auto",
        devices="auto",
    ...
    val_metrics: typing.Dict[str, float] = {
...
def tune(cfg: DictConfig) -> None:
    """GMD hyperparameter tuning entry point.

    Dispatches to Lightning Tuner or Ray Tune based on
    ``hparams_search.method``.
    """
    hp_cfg: typing.Optional[DictConfig] = cfg.get("hparams_search")
    if hp_cfg is None:
        raise ValueError(
            "No hparams_search config found. Use:\n"
            "  gmd-tune hparams_search=basic     # Lightning Tuner\n"
            "  gmd-tune hparams_search=ray_tune   # Ray Tune"
        )

    method: str = hp_cfg.get("method", "tuner")

    if method == "tuner":
        from gmd.training.tuning import run_lightning_tuner
        ...
        module: GMDModule = GMDModule(
            backbone=backbone, head=head, loss=loss, config=cfg,
        )
        datamodule: GMDDataModule = GMDDataModule(cfg)
        trainer: L.Trainer = hydra.utils.instantiate(cfg.trainer)

        results: typing.Dict[str, typing.Any] = run_lightning_tuner(
            module, datamodule, trainer, cfg,
        )
        print(f"\n[Tuner results] {results}")

    elif method == "ray":
        from gmd.training.tuning import run_ray_tune
        ...
        from gmd.training.tuning import run_wandb_sweep
        ...
        def _wandb_train_fn() -> None:
            ...
            with wandb.init() as run:
                ...
                datamodule = GMDDataModule(merged)
                ...
                trainer = L.Trainer(
                    max_epochs=int(
                        merged.training.get("max_epochs", 500)
                    ),
                    accelerator="auto",
                    devices="auto",
                    logger=wandb_logger,
        ...
        raise ValueError(
            f"Unknown tuning method '{method}'. Use 'tuner', 'ray', or 'wandb'."
        )
