...
Wraps a pre-trained MACE model as a GMD backbone. Uses MACE's own
...
def _require_mace() -> None:
    ...
    try:
        ...
        raise ImportError(
            ...
            "Or: pip install gmd[mace]"
        ) from None


class MACEAdapter:
    """Wraps a pre-trained MACE foundation model as a GMD backbone.
    ...
    def _to_mace_batch(self, graph: AtomicGraph) -> typing.Dict[str, typing.Any]:
        """Convert GMD AtomicGraph to MACE's expected input format."""
        return {
    ...
    def extract_features(
        ...
        Use ``gmd.utils.extraction.extract_scalars()`` to get invariant
