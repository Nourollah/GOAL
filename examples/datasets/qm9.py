...
QM9_TARGETS: typing.Dict[int, str] = {
...
# Reverse: name → index
_QM9_TARGETS_BY_NAME: typing.Dict[str, int] = {v: k for k, v in QM9_TARGETS.items()}
...
class QM9Dataset(BenchmarkDataset):
    ...
    def __init__(
        self,
        root: str,
        target: typing.Union[str, int, None] = "homo",
        cutoff: float = 5.0,
        split: str = "train",
        seed: int = 42,
        ...
        self.target_idx: typing.Optional[int] = self._resolve_target(target)
        self.target_name: typing.Optional[str] = (
    ...
    def _resolve_target(
        ...
        if isinstance(target, int):
            if target not in QM9_TARGETS:
                raise ValueError(
                    f"QM9 target index must be 0–18, got {target}"
                )
            return target
        if target in _QM9_TARGETS_BY_NAME:
            return _QM9_TARGETS_BY_NAME[target]
        raise ValueError(
            f"Unknown QM9 target '{target}'. "
            f"Available: {list(_QM9_TARGETS_BY_NAME.keys())}"
        )
    ...
    def _download_and_process(self) -> typing.List[typing.Any]:
        ...
        for data in pyg_dataset:
            ...
            energy: typing.Optional[torch.Tensor] = None
