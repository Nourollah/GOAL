...
class TestBuildStrategy:
    ...
    def test_fsdp2_strategy(self):
        ...
        cfg: DictConfig = OmegaConf.create({"strategy": {"name": "fsdp2"}})
        result: typing.Any = build_strategy(cfg)
    ...
    def test_deepspeed_missing_raises(self):
        ...
        cfg: DictConfig = OmegaConf.create({"strategy": {"name": "deepspeed_zero2", "stage": 2}})

        with patch.dict("sys.modules", {"deepspeed": None}):
            with patch(
                "goal.ml.training.strategies.deepspeed._require_deepspeed",
...
class TestKnownStrategies:
    ...
    def test_known_strategies_list(self):
        ...
        from goal.ml.training.strategies.factory import _KNOWN_STRATEGIES
