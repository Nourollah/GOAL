...
class MockAtomicDataset(Dataset):
    ...
    def __init__(
        self,
        root: typing.Union[str, Path] = "/fake",
        cutoff: float = 5.0,
        split: str = "train",
        **kwargs: typing.Any,
    ) -> None:
...
class TestResolvePaths:
    ...
    def test_single_string(self):
        from gmd.data.datamodule import _resolve_paths

        result: typing.List[str] = _resolve_paths("/some/path.xyz")
        assert result == ["/some/path.xyz"]

    def test_list_of_strings(self):
        from gmd.data.datamodule import _resolve_paths

        result: typing.List[str] = _resolve_paths(["/a.xyz", "/b.xyz"])
    ...
    def test_omegaconf_listconfig(self):
        ...
        result: typing.List[str] = _resolve_paths(raw)
...
class TestDiscoverFilesInDir:
    ...
    def test_finds_supported_files(self, tmp_path: Path):
        ...
        from gmd.data.datamodule import _discover_files_in_dir
        ...
        found: typing.List[str] = _discover_files_in_dir(tmp_path)
        assert len(found) == 3
    ...
    def test_sorted_output(self, tmp_path: Path):
        ...
        found: typing.List[str] = _discover_files_in_dir(tmp_path)
        names: typing.List[str] = [Path(f).name for f in found]
...
class TestConcatAtomicDataset:
    ...
    def test_sequential_concat(self):
        ...
        from gmd.data.datasets.concat import ConcatAtomicDataset
...
class TestMergeDatasets:
    ...
    def test_single_dataset_returns_directly(self):
        ...
        from gmd.data.datamodule import _merge_datasets
...
# ---------------------------------------------------------------------------
# Tests: GMDDataModule — all 4 loading modes
# ---------------------------------------------------------------------------


class TestGMDDataModule:
    """Verify the GMDDataModule across loading modes."""
    ...
    def test_mode1_auto_split(self):
        ...
        with patch(
            "gmd.data.datamodule._build_datasets",
            return_value=[MockAtomicDataset(size=100)],
        ):
            dm: GMDDataModule = GMDDataModule(cfg)
            ...
            with patch.object(
                dm, "_try_named_splits", side_effect=FileNotFoundError("forced"),
            ):
                dm.setup(stage="fit")
    ...
    def test_meta_keys_include_dir_options(self):
        ...
        assert "train_dir" in GMDDataModule._META_KEYS
        assert "val_dir" in GMDDataModule._META_KEYS
        assert "test_dir" in GMDDataModule._META_KEYS
    ...
    def test_extra_kwargs_excludes_meta_keys(self):
        ...
        dm: GMDDataModule = GMDDataModule(cfg)
        extra: typing.Dict[str, typing.Any] = dm._extra_kwargs()
